...
Что проверяем по протоколам:

  • IKEv2 — ICMP-пинг IP-адреса;
  • AmneziaWG — TCP-порт 22 (сам WG на UDP, снаружи его не пощупать,
    поэтому о доступности узла судим по управляющему TCP-порту);
  • VLESS — домен по TCP: порт 22 плюс рабочий порт из настроек сервера,
    если он там указан.

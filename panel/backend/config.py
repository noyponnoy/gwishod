...
# Тот же BOT_TOKEN, что у админ-бота (проверка initData).
TELEGRAM_BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN", os.getenv("BOT_TOKEN", "")).strip()
# ID админов TG через запятую (как ADMIN_IDS у бота).
_tg_admins_raw = os.getenv("TELEGRAM_ADMIN_IDS", os.getenv("ADMIN_IDS", ""))
TELEGRAM_ADMIN_IDS: list[int] = []
for _x in _tg_admins_raw.replace(" ", "").split(","):
    if _x.isdigit():
        TELEGRAM_ADMIN_IDS.append(int(_x))

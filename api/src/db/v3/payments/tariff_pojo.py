...
class TariffPojo:
    ...
    def from_doc(doc: dict) -> "TariffPojo":
        ...
        t.technical_name = doc.get("technical_name", "0")
    ...
    async def delete(self) -> bool:
        collection = await self.get_collection()
        await collection.delete_one({"technical_name": self.technical_name})
        return True

    async def find(self) -> Optional["TariffPojo"]:
        collection = await self.get_collection()
        doc = await collection.find_one({"technical_name": self.technical_name})
    ...
    async def find_all() -> list["TariffPojo"]:
        ...
        async for doc in cursor:
            t = TariffPojo.from_doc(doc)
            if t.enabled:
                tariffs.append(t)
        return tariffs

    @staticmethod
    async def find_all_admin() -> list["TariffPojo"]:
        collection = await TariffPojo.get_collection()
        cursor = collection.find({})
        tariffs = []
        async for doc in cursor:

...
class ServerPojo:
    ...
    def from_doc(doc: dict) -> "ServerPojo":
        ...
        s.user = doc.get("user", "0")
        s.password = doc.get("password", "0")
        s.ca_file = doc.get("ca_file", "0")
    ...
    async def all_without_premium() -> list["ServerPojo"]:
        ...
        async for doc in cursor:
            ...
            if s.status:
                servers.append(s)
        premium_servers = await ServerPojo.find_with_premium()
        for ps in premium_servers:
            if ps.status:
                tmp = ServerPojo.from_doc(ps.to_doc())
                tmp.user = ""
                tmp.password = ""
                tmp.ca_file = ""
                servers.append(tmp)
